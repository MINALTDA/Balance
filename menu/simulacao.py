...
#     # --- Gráfico INTERATIVO comparativo: todo o período ---
#     st.subheader(f"Comparação de estratégias para {row['par']} (período completo)")
#     fig = go.Figure()
#     fig.add_trace(go.Scatter(x=df_lag.index, y=df_lag["Cumulativo"], mode='lines', name=f"Lead-Lag ótimo (lag={row['lag']})"))
#     fig.add_trace(go.Scatter(x=df_exp.index, y=df_exp["Cumulativo"], mode='lines', name=f"Lead-Lag exploratório (lag={row['lag_exp']})", line=dict(dash='dash')))
#     fig.add_trace(go.Scatter(x=df_trad.index, y=df_trad["Cumulativo"], mode='lines', name="Tradicional (lag=0)", line=dict(dash='dot')))
#     fig.update_layout(
#         yaxis_title="Crescimento acumulado (R$ inicial = 1)",
#         title=f"Crescimento acumulado: {row['par']}",
#         legend=dict(itemsizing='constant'),
#         height=400
#     )
...
#     # --- Teste fora da amostra: último ano (SEM DESAPARECER) ---
#     st.subheader("Comparação fora da amostra (último ano)")
#     all_dates = df_lag.index
#     if len(all_dates) < 252:
#         st.warning("Não há histórico suficiente para realizar teste fora da amostra (último ano).")
...
# # Comissão fixa para toda a plataforma
...
#     # --- Sidebar compacto: apenas ativos ---
#     with st.sidebar:
...
#     # --- Demais parâmetros em painel colapsável ---
...
# # Paleta de cores e símbolos única para sinais
# SIGNAL_STYLE = {
#     "long_open":    {"symbol": "triangle-up",   "color": "#15803d", "label": "Abrir Long",  "showlegend": True},
#     "long_close":   {"symbol": "circle",        "color": "#65e38c", "label": "Fechar Long", "showlegend": True},
#     "short_open":   {"symbol": "triangle-down", "color": "#c81e1e", "label": "Abrir Short", "showlegend": True},
#     "short_close":  {"symbol": "circle",        "color": "#f87171", "label": "Fechar Short","showlegend": True},
# }

# def show_signals(fig, df, sinais, prefix=""):
#     # Adiciona cada tipo de sinal só uma vez na legenda!
#     signal_done = set()
#     for typ, idx, val in sinais:
#         stl = SIGNAL_STYLE[typ]
#         legend_name = stl["label"] if stl["label"] not in signal_done else None
...
#     fig = go.Figure()
#     # Linhas principais
#     fig.add_trace(go.Scatter(x=df_lag.index, y=df_lag["Cumulativo"], mode='lines', name=f"Lead-Lag ótimo (lag={row['lag']})", line=dict(color="blue", width=2)))
#     fig.add_trace(go.Scatter(x=df_exp.index, y=df_exp["Cumulativo"], mode='lines', name=f"Lead-Lag exploratório (lag={row['lag_exp']})", line=dict(color="orange", width=2, dash="dash")))
#     fig.add_trace(go.Scatter(x=df_trad.index, y=df_trad["Cumulativo"], mode='lines', name="Tradicional (lag=0)", line=dict(color="black", width=2, dash="dot")))

#     # Adiciona apenas os sinais para a estratégia principal selecionada
#     sinais = get_signals(df_lag, z_entry, z_exit)
#     show_signals(fig, df_lag, sinais)
...
#         if res_lag_test:
#             df = res_lag_test["df"]
#             fig2.add_trace(go.Scatter(x=df.index, y=df["Cumulativo"], mode='lines', name=f"Lead-Lag ótimo (lag={lag_trading_train})", line=dict(color="blue", width=2)))
#             sinais = get_signals(df, z_entry, z_exit)
#             show_signals(fig2, df, sinais)
#         if res_exp_test:
#             df = res_exp_test["df"]
#             fig2.add_trace(go.Scatter(x=df.index, y=df["Cumulativo"], mode='lines', name=f"Lead-Lag exploratório (lag={lag_exp_train})", line=dict(color="orange", width=2, dash="dash")))
#         if res_trad_test:
#             df = res_trad_test["df"]
#             fig2.add_trace(go.Scatter(x=df.index, y=df["Cumulativo"], mode='lines', name="Tradicional (lag=0)", line=dict(color="black", width=2, dash="dot")))
...
#     # showlegend=True apenas para mostrar la marca en la leyenda UNA vez por tipo
#     legends_drawn = set()
#     for typ, idx, val in sinais:
#         stl = SIGNAL_STYLE[typ]
...
#     # --- Gráfico 1: sem marcas, só linhas ---
#     st.subheader(f"Comparação de estratégias para {row['par']} (período completo)")
#     fig = go.Figure()
#     fig.add_trace(go.Scatter(x=df_lag.index, y=df_lag["Cumulativo"], mode='lines', name=f"Maximizar sharpe (lag={row['lag']})", legendgroup="LeadLag", line=dict(color="blue", width=2)))
#     fig.add_trace(go.Scatter(x=df_exp.index, y=df_exp["Cumulativo"], mode='lines', name=f"Minimizar autocorrelação (lag={row['lag_exp']})", legendgroup="Exploratorio", line=dict(color="orange", width=2, dash="dash")))
#     fig.add_trace(go.Scatter(x=df_trad.index, y=df_trad["Cumulativo"], mode='lines', name="Tradicional (lag=0)", legendgroup="Tradicional", line=dict(color="black", width=2, dash="dot")))
#     fig.update_layout(
#         yaxis_title="Crescimento acumulado (R$ inicial = 1)",
#         title=f"Crescimento acumulado: {row['par']}",
#         legend=dict(itemsizing='constant'),
...
#     st.subheader("Simulação do último ano")
#     all_dates = df_lag.index
#     if len(all_dates) < 252:
#         st.warning("Não há histórico suficiente para realizar teste fora da amostra (último ano).")
#     else:
...
#         # Leyenda: marcas aparecen sólo una vez, asociadas a la primera estrategia.
#         fig2.update_layout(
#             title=f"Teste fora da amostra - Último ano: {row['par']}",
#             yaxis_title="Crescimento acumulado (R$ inicial = 1)",
#             legend=dict(itemsizing='constant'),
...
#     st.subheader(f"Comparação de estratégias para {row['par']} (período completo)")
#     fig = go.Figure()
#     fig.add_trace(go.Scatter(x=df_lag.index, y=df_lag["Cumulativo"], mode='lines', name=nome_sharpe, legendgroup="LeadLag", line=dict(color="blue", width=2)))
#     fig.add_trace(go.Scatter(x=df_exp.index, y=df_exp["Cumulativo"], mode='lines', name=nome_autocorr, legendgroup="Exploratorio", line=dict(color="orange", width=2, dash="dash")))
#     fig.add_trace(go.Scatter(x=df_trad.index, y=df_trad["Cumulativo"], mode='lines', name=nome_trad, legendgroup="Tradicional", line=dict(color="black", width=2, dash="dot")))
#     fig.update_layout(
#         yaxis_title="Crescimento acumulado (R$ inicial = 1)",
#         title=f"Crescimento acumulado: {row['par']}",
#         legend=dict(itemsizing='constant'),
...
#     st.markdown("#### Métricas detalhadas")
#     col1, col2, col3 = st.columns(3)
#     col1.metric(f"Sharpe - {nome_sharpe}", f"{row['sharpe_lag']:.2f}")
#     col1.metric(f"Crescimento - {nome_sharpe}", f"{row['crescimento_lag']:.2f}x")
#     col2.metric(f"Sharpe - {nome_autocorr}", f"{row['sharpe_exp']:.2f}")
#     col2.metric(f"Crescimento - {nome_autocorr}", f"{row['crescimento_exp']:.2f}x")
#     col3.metric(f"Sharpe - {nome_trad}", f"{row['sharpe_trad']:.2f}")
#     col3.metric(f"Crescimento - {nome_trad}", f"{row['crescimento_trad']:.2f}x")
#     st.caption(
#         "Compare como a estratégia muda ao utilizar o lag ótimo exploratório (científico), o ótimo da estratégia e o enfoque tradicional. "
#         "Resultados 100% baseados em dados históricos reais."
#     )
...
#         # Leyenda: marcas aparecem só uma vez.
#         fig2.update_layout(
#             title=f"Teste fora da amostra - Último ano: {row['par']}",
#             yaxis_title="Crescimento acumulado (R$ inicial = 1)",
#             legend=dict(itemsizing='constant'),
