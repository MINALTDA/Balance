...
# # Lista ampla de ações US (S&P500). Puedes añadir más si deseas.
# TICKERS_LIST = [
...
# def get_valid_random_stocks(n=50, days=30):
#     tickers_test = random.sample(TICKERS_LIST, min(len(TICKERS_LIST), n+20))
#     data = yf.download(tickers_test, period=f'{days+10}d', interval='1d', progress=False)
#     # Usa 'Adj Close' si existe, senão 'Close'
#     if 'Adj Close' in data:
#         prices = data['Adj Close'].dropna(axis=1, how='any')
#     elif 'Close' in data:
#         st.warning("Usando 'Close' porque 'Adj Close' não está disponível para esses tickers.")
#         prices = data['Close'].dropna(axis=1, how='any')
#     else:
#         st.error("Nenhuma coluna 'Adj Close' ou 'Close' encontrada. Verifique os tickers ou a conexão.")
#         st.stop()
#     valid_tickers = list(prices.columns)
#     if len(valid_tickers) < n:
#         st.warning(f"Apenas {len(valid_tickers)} ações válidas. Usando todas.")
#         n = len(valid_tickers)
#     chosen = random.sample(valid_tickers, n)
#     final_data = prices[chosen].dropna(axis=0, how='any')[-days:]
#     return chosen, final_data

# def best_alignment_greedy(data):
#     # Heurística: alinhar maximizando a soma das correlações consecutivas
...
#     st.write(
#         "Seleciona aleatoriamente 50 ações do mercado americano, verifica quais possuem dados válidos e explora como alinhá-las para maximizar a soma das correlações entre pares consecutivos. Observe a diferença visual e estatística do alinhamento."
#     )
#     num_stocks = st.slider("Número de ações para explorar", 10, 50, 30)
#     tickers, data = get_valid_random_stocks(num_stocks, days=30)
#     st.write("Ações válidas selecionadas:", ", ".join(tickers))
#     st.dataframe(data)
...
#     # Soma das correlações pares consecutivos (antes e depois)
#     orig_corr_sum = sum([abs(data[t].corr(data[s])) for t, s in zip(data.columns[:-1], data.columns[1:])])
#     aligned_corr_sum = sum([abs(data[t].corr(data[s])) for t, s in zip(aligned_order[:-1], aligned_order[1:])])
#     st.success(f"**Soma das correlações pares consecutivos - original:** {orig_corr_sum:.2f}")
#     st.success(f"**Soma das correlações pares consecutivos - alinhado:** {aligned_corr_sum:.2f}")
...
#     st.title("Exploração: Alineamento Ótimo de Séries de Ações")
#     st.write(
#         "Seleciona aleatoriamente ações do mercado americano, verifica quais possuem dados válidos e explora como alinhá-las para maximizar a soma e a média das correlações entre pares consecutivos. Observe a diferença visual e estatística do alinhamento."
#     )
#     num_stocks = st.slider("Número de ações para explorar", 10, 50, 30)
#     tickers, data = get_valid_random_stocks(num_stocks, days=30)
#     st.write("Ações válidas selecionadas:", ", ".join(tickers))
...
#     st.subheader("Séries não alinhadas (ordem aleatória)")
#     plot_series(data, title="Séries originais")
#     orig_corr_sum, orig_corr_mean = calc_corr_metrics(data)
    
#     st.subheader("Séries alinhadas (máxima correlação entre pares consecutivos)")
#     aligned_order = best_alignment_greedy(data)
#     plot_series(data, order=aligned_order, title="Séries alinhadas pela heurística")
#     aligned_corr_sum, aligned_corr_mean = calc_corr_metrics(data, order=aligned_order)
    
#     st.markdown("### Estatísticas de correlação entre pares consecutivos")
#     col1, col2 = st.columns(2)
#     with col1:
#         st.info(f"**Soma das correlações - original:** {orig_corr_sum:.2f}")
#         st.info(f"**Média das correlações - original:** {orig_corr_mean:.3f}")
#     with col2:
#         st.success(f"**Soma das correlações - alinhado:** {aligned_corr_sum:.2f}")
...
#     """Tenta encontrar n tickers válidos (com dados Close ou Adj Close), tentando diferentes combinações aleatórias."""
#     tried = set()
#     valid_tickers = []
#     attempt = 0
...
#     # Si no se encontró ningún ticker válido, detener la ejecución y mostrar error:
...
#     # Si no consiguió n, usa el máximo posible
#     if len(valid_tickers) < n:
...
#         final_prices = final_data['Adj Close'][chosen].dropna(axis=0, how='any')[-days:]
#     elif 'Close' in final_data:
#         final_prices = final_data['Close'][chosen].dropna(axis=0, how='any')[-days:]
#     else:
#         st.error("Não foi possível baixar preços finais das ações selecionadas.")
#         st.stop()
#     # Si no hay datos, también detener:
#     if final_prices.empty:
#         st.error("Não há dados suficientes das ações selecionadas. Tente novamente ou use um número menor de ações.")
#         st.stop()
...
#     )
#     num_stocks = st.slider("Número de ações para explorar", 10, 50, 30)
#     tickers, data = get_valid_random_stocks(num_stocks, days=750)
#     st.write(f"Ações válidas selecionadas ({len(tickers)}):", ", ".join(tickers))
#     st.dataframe(data)
