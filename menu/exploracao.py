...
# def show():
#     st.title("Correlación y Previsión: Acciones y Random Forest")
#     st.write("1. Descubre las 7 acciones más correlacionadas con la acción objetivo (usando *lag*).\n"
#              "2. Haz previsión de la acción objetivo usando Random Forest y las 7 seleccionadas como predictores.")
...
#     st.markdown("## Previsión de la Acción Objetivo usando Random Forest")
#     dias_pred = st.slider("¿Cuántos días futuros predecir? (test set)", 1, 15, 7)
#     # Número de días a usar para ajuste de X, y
#     total_dias = dias_historico
#     dias_train_valid = total_dias - dias_pred
#     if dias_train_valid < 30:
#         st.warning("Por favor selecciona más días históricos o menos días de predicción.")
#         return

#     # Construir X (top7 alineadas por lag óptimo), y (target)
#     # Solo se consideran los últimos 'total_dias' días
#     # Cada predictor ya tiene lag incorporado
#     X = np.column_stack([r["serie_lag"] for r in top7])
#     y = target

#     # División entrenamiento-validación
#     split = int(0.7 * dias_train_valid)
#     X_train, X_valid = X[:split], X[split:dias_train_valid]
#     y_train, y_valid = y[:split], y[split:dias_train_valid]
...
#     # Predicción real (hacia los últimos N días)
#     X_test = X[dias_train_valid:]
#     y_test = y[dias_train_valid:]
#     y_pred = rf.predict(X_test)

#     # Mostrar métricas
#     st.markdown(f"**MSE validación:** {mse:.2f} | **R² validación:** {r2:.2f}")

#     # Gráfico real vs previsto (últimos N días + 15 extra si se puede)
#     extra = 15
#     dias_graf = dias_pred + extra
#     idx_ini = max(0, len(y) - dias_graf)
#     real_graf = y[idx_ini:]
#     pred_graf = np.concatenate([np.full(dias_graf - dias_pred, np.nan), y_pred]) if len(y_pred) < dias_graf else y_pred[-dias_graf:]
...
#     ))
#     fig_pred.update_layout(title=f"Previsión Random Forest ({dias_pred} días + {extra} días extra)",
#                           xaxis_title="Día",
#                           yaxis_title="Precio",
#                           height=450)
