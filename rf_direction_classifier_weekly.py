# rf_direction_classifier_weekly.py
...
st.set_page_config(page_title="Random Forest Dirección Semanal", layout="wide")
st.title("📅 Random Forest para Dirección Semanal (Sube/Baja)")
...
max_lag = st.sidebar.slider("Máximo lag por ticker", 1, 8, 4)
...
st.write(f"**Objetivo:** predecir si {target} sube o baja la próxima semana usando retornos y lags semanales.")
...
# 3. Cálculo de retornos semanales y features
...
for ticker in tickers:
    for lag in range(1, max_lag+1):
        ...
        features.append(col)
rets["target_up"] = (rets[target].shift(-1) > 0).astype(int)  # Dirección para la próxima semana
...
# 5. Random Forest
rf = RandomForestClassifier(n_estimators=n_estimators, max_depth=5, random_state=42, n_jobs=-1)
rf.fit(X_train, y_train)
y_pred = rf.predict(X_test)
y_proba = rf.predict_proba(X_test)[:, 1]

# 6. Métricas
st.subheader("Métricas de clasificación semanal")
...
st.caption("Si accuracy y AUC mejoran, puedes intentar mensual, añadir indicadores técnicos o umbrales de decisión.")
